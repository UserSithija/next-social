...
export const switchFollow = async (userId: string) => {
  ...
  try {
    ...
    if (existingFollow) {
      ...
      const existingFollowReqest = await prisma.followRequest.findFirst({
        where: {
      ...
      if (existingFollowReqest) {
        await prisma.followRequest.delete({
          where: {
            id: existingFollowReqest.id,
          },
        });
      } else {
...
export const switchBlock = async (userId: string) => {
  ...
  try {
    const existtingBlock = await prisma.block.findFirst({
      where: {
